...
-- Enable break indent - match the indentation of the original line
...
-- Checks if you have installed `lazy` and installs it if you haven't
require("lazy").setup({
	...
	{ -- Autoformat - this will format using the external tool we have set up, but if it's not available, then it will fall back to LSP formatting
	...
	{ -- Autocompletion - in this configuration the navigation keys are not the standart, but probably better than using Tab for a hundred different things
		"saghen/blink.cmp",
		event = "VimEnter",
		version = "1.*",
		dependencies = {
